...
class BothandlerPlugin(b3.plugin.Plugin):
    ...
    def addBots(self):
        ...
        if self._botstart: 
            ...
            elif bclients > 0: # Check if we need to add bots
    ...
    def cmd_addbots(self, data, client, cmd=None):
        """\
        Add bots to server and turn on regulation. <number> to add extra bots with regulation is off.
        """
        input = self._adminPlugin.parseUserCmd(data)
        if not input:
            self._botstart = True
